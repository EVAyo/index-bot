    ...
    fun executeByEnrollButton(type: Type, request: RequestService.BotRequest) {
        ...
        when {
            ...
            field == "fail" -> {
                ...
                awaitStatusService.clearAwaitStatus(request.chatId!!)
                val manager = request.update.callbackQuery().from()
                val msg = recordMsgFactory.makeApproveFailReasonSelectMsg(request.chatId!!, enroll, manager)
                botProvider.send(msg)
            }
            // 不通过 - 原因
            field == "fail-reason" -> {
                botProvider.sendDeleteMessage(request.chatId!!, request.messageId!!)
                awaitStatusService.clearAwaitStatus(request.chatId!!)
                val reasonType = callbackDataVal[2]
                if (reasonType == "back") {
                    val msg = recordMsgFactory.makeApproveMsg(request.chatId!!, enroll)
                    botProvider.send(msg)
                    return
                }
                val manager = request.update.callbackQuery().from()
                val reason = when (reasonType) {
                    "require" -> "暂未达到收录条件，为保障索引质量，请继续运营一段时间后再次提交。如有疑问请咨询审核员。"
                    "detail" -> "提交收录的频道或群组主题与您填写的信息不符，请修改或咨询审核员后重试。"
                    "black" -> "暂不支持博彩、公开他人隐私、推广、批量账号交易、暗网和黑灰色产业链等的标签。"
                    "nsfw" -> "受黑灰产渗透影响，不再支持收录新的 NSFW 群组和频道。"
                    "type" -> "不支持收录此类型的群组和频道。"
                    "other" -> "当前状况有些棘手，建议您联系审核员进行沟通。"
                    else -> "未知"
                }
                enrollService.approveEnroll(enroll.uuid, manager, false, reason)
            }
        }
    }
